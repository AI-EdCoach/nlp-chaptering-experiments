...
class ChapterEstimator:
    def __init__(self, model_checkpoint: str = 'paraphrase-multilingual-MiniLM-L12-v2',
                 hf_model: str = "bert-base-multilingual-cased", threshold: float | None = None):
        self.model = SentenceTransformer(model_checkpoint)
        self.hf_model = pipeline("feature-extraction", model=hf_model)
        self.threshold = threshold

    def __call__(self, text: Union[str, List[str]], **kwargs) -> pd.DataFrame:
        ...
        distances = []
        values_dict = {"index": [i for i in range(len(sentences))], "sentence": sentences,
        ...
        for i in range(1, len(embeddings)):
            sim = cosine_similarity(embeddings[1-i].reshape(1, -1), embeddings[i].reshape(1, -1))[0][0]
            cos_dist.append(sim)
        ...
        if threshold is None:
            threshold = df['super_score'].quantile(.10)
        df['chapter'] = [1 if score < threshold else 0 for score in df['super_score']]
        return df[['index', 'sentence', 'super_score', 'chapter']]
